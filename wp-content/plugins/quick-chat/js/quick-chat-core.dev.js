                                    ...
                                    for (var mes_room_name in sys_mes){
                                        if(sys_mes[mes_room_name]['private_to'] == quick_chat.user_name && sys_mes[mes_room_name]['type'] == 'INV'){ // Receiver
                                            if(quick_chat.is_private_allowed(updates[i].status)){
                                                ...
                                                quick_chat.update_private_cookie('quick_chat_private_queue', quick_chat.private_queue);
                                            ...
                                            for (var pq_room_name in quick_chat.private_queue){
                                                if(quick_chat.is_private_eq(quick_chat.private_queue[pq_room_name], sys_mes[mes_room_name])){
                                                    ...
                                                    quick_chat.update_private_cookie('quick_chat_private_current', quick_chat.private_current);
...
if(quick_chat.no_participation == 0){
    if(jQuery.cookie('quick_chat_private_queue') != null)
        quick_chat.private_queue = jQuery.parseJSON(jQuery.cookie('quick_chat_private_queue'));

    if(jQuery.cookie('quick_chat_private_current') != null)
        quick_chat.private_current = jQuery.parseJSON(jQuery.cookie('quick_chat_private_current'));
...
jQuery("div.quick-chat-smile").bind('click', function() {
    ...
    jQuery(this).fadeTo(100, 0, function() {
        jQuery(input_textarea).quick_chat_insert_at_caret(jQuery(this_element).attr('title'));
