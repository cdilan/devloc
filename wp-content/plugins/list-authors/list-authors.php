...
if ( version_compare( $GLOBALS['wp_version'], '2.8', '>=' ) ) {
	...
	function wp_list_authors2($args = '') {
		...
		$defaults = array(
			...
			'order' => 'DESC', 'number' => NULL, 'min_count' => NULL
		);
		...
		if ( $show_fullname ) {
			...
			if ( $orderby == 'count' ) {
				$author_sort = "ORDER BY post_count $order, author_name $order";
			} else {
				$author_sort = "ORDER BY $wpdb->users.ID desc";
			}
		}
		...
		if ( !$hide_empty && !$min_count ) {
			$author_cap_join = "JOIN $wpdb->usermeta AS meta_capabilities ON meta_capabilities.user_id = $wpdb->users.ID 
			AND meta_capabilities.meta_key = '{$blog_prefix}capabilities'
			AND meta_capabilities.meta_value like '%jogador%'";
		} else {
			$author_cap_join = '';
		}
